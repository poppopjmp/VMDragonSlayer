...
try:
    from PySide2.QtWidgets import (
        ...
        QSplitter, QTextEdit, QLabel, QGroupBox, QPushButton, QComboBox,
...
class VMComponentNode:
    ...
    def __init__(self, name: str, component_type: str, confidence: float = 0.0,
                 address: Optional[int] = None, size: Optional[int] = None):
        self.name = name
...
class VMArchitectureTree(QTreeWidget):
    ...
    def load_vm_structure(self, vm_analysis_data: Dict[str, Any]):
        ...
        try:
            ...
            self._add_interpreter_loop(root_item, vm_analysis_data.get('interpreter', {}))
            self._add_memory_layout(root_item, vm_analysis_data.get('memory_layout', {}))
...
class ComponentDetailsPanel(QWidget):
    ...
    def update_component(self, component_data: Dict[str, Any]):
        ...
        for row, pattern in enumerate(patterns):
            self.patterns_table.setItem(row, 0, QTableWidgetItem(pattern.get('name', '')))
...
class VMStructureExplorer(QWidget):
    ...
    def setup_ui(self):
        ...
        # Connect signals
        self.architecture_tree.component_selected.connect(self.details_panel.update_component)
...
if not QT_AVAILABLE:
    class VMStructureExplorer:
        ...
        def load_vm_analysis(self, analysis_data: Dict[str, Any]):
            self.vm_data = analysis_data
            logging.info(f"VM Structure Explorer: Loaded analysis data with {len(analysis_data)} components")
