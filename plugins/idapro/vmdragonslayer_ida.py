    ...
    def _is_potential_vm_handler(self, func_ea, func_size):
        ...
        for head in idautils.Heads(func_ea, func_ea + func_size):
            if idc.is_code(idc.get_full_flags(head)):
                ...
                if mnem == 'jmp' and idc.get_operand_type(head, 0) in [idc.o_phrase, idc.o_displ]:
                    has_indirect_jumps = True
                
                # Count register operations
                if any(reg in idc.print_operand(head, 0) for reg in ['eax', 'ebx', 'ecx', 'edx']):
                    register_usage += 1
...
class VMStructureDiscovery:
    ...
    def _find_vm_context(self, handlers):
        ...
        for handler in handlers:
            for head in idautils.Heads(handler['address'], 
                                     handler['address'] + handler['size']):
                if idc.is_code(idc.get_full_flags(head)):
...
class VMDragonSlayerPlugin(idaapi.plugin_t):
    ...
    def _initialize_ui_components(self):
        ...
        ui_components = {
            ...
            'config_dialog': ConfigurationDialog(
                self.config, self.core_services
            ),
    ...
    def get_ui_status_summary(self):
        ...
        status_summary = self.ui_components['status_indicators'].get_service_status_summary()
        metrics = self.ui_components['metrics_dashboard'].update_metrics_display()
        active_operations = self.ui_components['progress_tracker'].get_active_operations()
    ...
    def show_ui_dashboard(self):
        ...
        try:
            ...
            status_indicators = self.ui_components['status_indicators'].update_service_status_indicators()
            metrics = self.ui_components['metrics_dashboard'].update_metrics_display()
            service_controls = self.ui_components['service_manager'].create_service_controls()
            ...
            print(f"Services: {ui_status['service_status']['available']}/{ui_status['service_status']['total']} available")
    ...
    def _show_config_dialog(self):
        ...
        dlg = ida_kernwin.Form(form, {
            'chkDTT': ida_kernwin.Form.ChkGroupControl(("DTT", "SE", "Anti", "VM")),
            'chkSE': ida_kernwin.Form.ChkGroupControl(("DTT", "SE", "Anti", "VM")),
            'chkAnti': ida_kernwin.Form.ChkGroupControl(("DTT", "SE", "Anti", "VM")),
            'chkVM': ida_kernwin.Form.ChkGroupControl(("DTT", "SE", "Anti", "VM")),
            'intTimeout': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_UINT32),
            'intHandlers': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_UINT32),
            'floatConfidence': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_FLOAT),
    ...
    def _initialize_engines(self):
        ...
        if self.core_services.unified_api:
            try:
                ...
                self.analysis_engines['orchestrator'] = self.core_services.get_service('orchestrator')
                self.analysis_engines['ml_engine'] = self.core_services.get_service('ml_engine')
                self.analysis_engines['dtt_executor'] = self.core_services.get_service('dtt_executor')
    ...
    def analyze_with_core_services(self):
        ...
        try:
            ...
            available_services = sum(1 for available in service_status.values() if available)
            ...
            print(f"Core services status: {available_services}/{total_services} available")
            ...
            if pattern_db and handlers:
                for i, handler in enumerate(handlers):
                    try:
                        patterns = pattern_db.match_patterns(handler.get('instructions', []))
                        ...
                        print(f"Pattern matching failed for handler {handler.get('name', 'unknown')}: {e}")
                    
                    # Update sub-progress
                    handler_progress = 20 + (15 * (i + 1) / len(handlers))
                    self.ui_components['progress_tracker'].update_progress(
                        operation_id, handler_progress, f"Processing handler {i+1}/{len(handlers)}"
                    )
            ...
            if structure_analyzer:
                ...
                if validation_framework:
                    try:
                        ...
                        print(f"✓ VM structure validation score: {validation_result.get('score', 0.0):.2f}")
                    except Exception as e:
            ...
            self.ui_components['progress_tracker'].complete_operation(
                operation_id, True, f"Analysis completed successfully with {len(handlers)} handlers"
            )
    ...
    def _finalize_analysis(self, results, start_time):
        ...
        if gpu_profiler:
            try:
                ...
                print(f"✓ GPU metrics: {gpu_metrics.get('execution_time', 0):.3f}s execution time")
            except Exception as e:
        ...
        if validation_framework:
            try:
                ...
                print(f"✓ Overall validation score: {overall_validation.get('score', 0.0):.2f}")
            except Exception as e:
    ...
    def _show_config_dialog(self):
        ...
        try:
            dlg = ida_kernwin.Form(form, {
                ...
                'chkAnti': ida_kernwin.Form.ChkGroupControl(("DTT", "SE", "Anti", "VM")),
                'chkVM': ida_kernwin.Form.ChkGroupControl(("DTT", "SE", "Anti", "VM")),
                'chkDB': ida_kernwin.Form.ChkGroupControl(("DB", "Val", "GPU", "Pat")),
                'chkValidation': ida_kernwin.Form.ChkGroupControl(("DB", "Val", "GPU", "Pat")),
                'chkGPU': ida_kernwin.Form.ChkGroupControl(("DB", "Val", "GPU", "Pat")),
                'chkPatterns': ida_kernwin.Form.ChkGroupControl(("DB", "Val", "GPU", "Pat")),
                'intTimeout': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_UINT32),
                'intHandlers': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_UINT32),
                'floatConfidence': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_FLOAT),
                'floatValidation': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_FLOAT),
                'strDBPath': ida_kernwin.Form.StringInput(),
                'chkAutoStore': ida_kernwin.Form.ChkGroupControl(("AutoStore",)),
                'intGPUDevice': ida_kernwin.Form.NumericInput(tp=ida_kernwin.Form.FT_UINT32),
                'rOutput': ida_kernwin.Form.RadGroupControl(("JSON", "XML", "HTML"))
            })
...
class StatusIndicatorPanel:
    ...
    def create_status_indicators(self):
        ...
        services = ['sample_database', 'validation_framework', 'gpu_profiler', 'pattern_database']
    ...
    def get_service_status_summary(self):
        ...
        available_services = sum(1 for indicator in self.indicators.values() 
...
class ConfigurationDialog:
    ...
    def create_enhanced_config_dialog(self):
        ...
        form = form_template % (
            ...
            db_status, val_status, gpu_status, pattern_status
        )
    ...
    def validate_configuration(self, config_data):
        ...
        if config_data.get('database_enabled') and not db_path:
            validation_results['warnings'].append("Database path is empty but database is enabled")
    ...
    def create_overall_metrics_panel(self):
        ...
        panel = {
            ...
            'metrics': {
                ...
                'system_health': 'Healthy' if available_services == total_services else 'Partial',
...
class ProgressTracker:
    ...
    def start_analysis_progress(self, operation_id, operation_name, estimated_duration=None):
    ...
    def update_progress(self, operation_id, percentage, current_step, steps_completed=None, total_steps=None):
        ...
        print(f"Progress update: {operation['name']} - {percentage:.1f}% - {current_step}")
    ...
    def complete_operation(self, operation_id, success=True, final_message=None):
        ...
        operation['progress_percentage'] = 100 if success else operation['progress_percentage']
        ...
        print(f"Operation completed: {operation['name']} - {'Success' if success else 'Failed'}")
...
class ServiceManagerPanel:
    ...
    def restart_service(self, service_name):
        ...
        try:
            ...
            if service and hasattr(service, 'restart'):
                ...
                if result:
                    self.log_service_action(service_name, 'restart', 'success', 'Service restarted successfully')
                    return True
                else:
                    self.log_service_action(service_name, 'restart', 'error', 'Service restart failed')
                    return False
            else:
                self.log_service_action(service_name, 'restart', 'warning', 'Service does not support restart')
                return False
                
        except Exception as e:
            self.log_service_action(service_name, 'restart', 'error', f'Restart failed: {e}')
            return False
        ...
        if service:
            try:
                ...
                if hasattr(service, 'get_performance_metrics'):
                    diagnostics['performance_metrics'] = service.get_performance_metrics()
    ...
    def get_service_logs(self, service_name, limit=20):
        ...
        return self.service_logs[service_name][-limit:] if limit else self.service_logs[service_name]
