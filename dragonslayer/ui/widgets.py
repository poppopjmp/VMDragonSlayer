...
class ChartWidget(BaseWidget):
    ...
    def render(self) -> Dict[str, Any]:
        chart_type = self.config.properties.get('chart_type', 'line')
                ...
                'children': [
                    {
                        ...
                        'children': [
                            {
                                'type': 'span',
                                'children': task.get('name', 'Task')
                            },
                            {
                                'type': 'span',
                                'children': f"{progress:.1f}%"
                            }
                        ]
                    },
                    {
...
class WidgetManager:
    ...
    def create_dashboard_layout(self, widget_ids: List[str], 
...
def create_metric_widget(title: str, value: Any, unit: str = '', 
                        trend: float = 0) -> WidgetConfig:
...
def create_chart_widget(title: str, chart_type: str = 'line') -> WidgetConfig:
    ...
    return WidgetConfig(
        title=title,
        widget_type='chart',
        properties={'chart_type': chart_type}
    )


def create_table_widget(title: str, headers: List[str], 
                       rows: List[List[Any]]) -> WidgetConfig:
    """Create configuration for a table widget."""
    return WidgetConfig(
        title=title,
        widget_type='table',
        properties={'headers': headers, 'rows': rows}
    )
