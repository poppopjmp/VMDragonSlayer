...
class ThreatIntelligenceProcessor:
    ...
    def search_threats(self, 
                      ...
                      days_back: int = 30) -> List[ThreatIntelligence]:
    ...
    def analyze_threat_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        if len(sorted_dates) >= 7:
            ...
            older_avg = sum(daily_counts[d] for d in sorted_dates[:-7]) / max(1, len(sorted_dates) - 7)
        ...
        return {
            ...
            "threat_summary": {
                ...
                "high_severity": len([t for t in recent_threats if t.severity.lower() == 'high']),
                "medium_severity": len([t for t in recent_threats if t.severity.lower() == 'medium']),
                "low_severity": len([t for t in recent_threats if t.severity.lower() == 'low'])
            },
            "family_distribution": [
    ...
    def _analyze_indicators(self, indicators: List[ThreatIndicator]) -> Dict[str, Any]:
        ...
        if not indicators:
            return {"total": 0, "types": {}, "confidence": {"avg": 0, "min": 0, "max": 0}}
    ...
    def _find_related_threats(self, threat: ThreatIntelligence, limit: int = 5) -> List[ThreatIntelligence]:
    ...
    def _calculate_similarity(self, threat1: ThreatIntelligence, 
        ...
        # Indicator overlap
        indicators1 = set(ind.indicator_value for ind in threat1.indicators)
        indicators2 = set(ind.indicator_value for ind in threat2.indicators)
    ...
    def _generate_threat_timeline(self, threat: ThreatIntelligence) -> List[Dict[str, Any]]:
        ...
        if threat.last_seen != threat.first_seen:
            timeline.append({
                ...
                "event": "Last Activity",
                "description": f"Most recent activity observed"
            })
        
        return timeline
    
    def _create_campaign_from_threats(self, family: str, 
                                    threats: List[ThreatIntelligence]) -> ThreatCampaign:
        """Create a threat campaign from grouped threats."""
        campaign_id = hashlib.md5(f"{family}_{threats[0].first_seen}".encode()).hexdigest()[:12]
...
def create_threat_indicator(indicator_type: str, value: str, family: str, 
                          confidence: float, source: str = "manual") -> ThreatIndicator:
...
def create_threat_intelligence(threat_type: str, family: str, severity: str,
                             indicators: List[ThreatIndicator],
                             description: str = "") -> ThreatIntelligence:
    """Create new threat intelligence entry."""
    threat_id = hashlib.md5(f"{family}_{threat_type}_{datetime.now()}".encode()).hexdigest()[:12]
