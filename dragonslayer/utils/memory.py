...
class MemoryManager:
        ...
        while not self._stop_monitoring.wait(30):  # Check every 30 seconds
            try:
                ...
                if usage.rss_mb > self.gc_threshold_mb:
                    logger.warning(f"Memory usage ({usage.rss_mb:.1f} MB) exceeds threshold "
    ...
    def memory_limit(self, limit_mb: int) -> ContextManager:
        ...
        logger.debug(f"Starting with memory limit: {limit_mb} MB "
        ...
        try:
            ...
            if final_usage.rss_mb > limit_mb:
                logger.warning(f"Memory limit exceeded: {final_usage.rss_mb:.1f} MB > {limit_mb} MB")
                self.optimize_memory()
...
def memory_monitor(limit_mb: Optional[int] = None) -> ContextManager:
    ...
    if limit_mb:
        ...
        try:
            ...
            logger.debug(f"Memory usage: {initial.rss_mb:.1f} MB → {final.rss_mb:.1f} MB "
...
def set_memory_limits(soft_limit_mb: int, hard_limit_mb: int) -> None:
    ...
    try:
        ...
        logger.info(f"Set memory limits: soft={soft_limit_mb}MB, hard={hard_limit_mb}MB")
