...
class EnsemblePredictor:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> Dict[str, float]:
        ...
        if results:
            ...
            self.best_model = getattr(self, f"{best_model_name}_classifier", None) or \
                             getattr(self, best_model_name, None)
            
            self.logger.info(f"Best model: {best_model_name} (score: {results[best_model_name]:.4f})")
    ...
    def get_feature_importance(self, model_name: Optional[str] = None) -> Optional[np.ndarray]:
        ...
        """
        if model_name:
            model = getattr(self, f"{model_name}_classifier", None) or \
