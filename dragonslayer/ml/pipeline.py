...
from .classifier import PatternClassifier, PatternFeatures, ClassificationResult
from .trainer import ModelTrainer, TrainingConfig, TrainingMetrics
from .model import MLModel, ModelType, ModelStatus, ModelRegistry
...
class FeatureExtractor:
    ...
    def extract_features(self, 
                        ...
                        symbolic_info: Optional[List[Dict]] = None) -> PipelineResult:
        """
        ...
        try:
            ...
            if len(bytecode_samples) < self.config.min_samples:
                logger.warning(f"Only {len(bytecode_samples)} samples provided, "
            ...
            for i, bytecode in enumerate(bytecode_samples):
                ...
                symbolic = symbolic_info[i] if symbolic_info and i < len(symbolic_info) else None
            ...
            if labels:
                ...
                if len(label_array) != len(feature_matrix):
                    raise MLError(f"Number of labels ({len(label_array)}) doesn't match "
    ...
    def _extract_single_sample(self, 
                             bytecode: bytes,
    ...
    def _analyze_control_flow(self, bytecode: bytes) -> float:
        ...
        jump_opcodes = {0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77,  # conditional jumps
                       0xEB, 0xE9, 0xEA, 0xFF}  # unconditional jumps
    ...
    def _remove_correlated_features(self, features: np.ndarray) -> np.ndarray:
        ...
        if len(keep_indices) < features.shape[1]:
            logger.info(f"Removed {features.shape[1] - len(keep_indices)} highly correlated features")
...
class MLPipeline:
    ...
    def run_training_pipeline(self,
                            ...
                            taint_info: Optional[List[Dict]] = None,
                            symbolic_info: Optional[List[Dict]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not feature_result.success:
                raise MLError(f"Feature extraction failed: {feature_result.error_message}")
    ...
    def _preprocess_features(self, features: np.ndarray, labels: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if SKLEARN_AVAILABLE:
            ...
            if self.config.use_feature_selection and X.shape[1] > self.config.feature_selection_k:
    ...
    def _train_models(self, X: np.ndarray, y: np.ndarray, validation_split: float) -> Dict[str, Any]:
    ...
    def _select_and_register_model(self, training_results: Dict, name: str, version: str) -> Dict[str, Any]:
