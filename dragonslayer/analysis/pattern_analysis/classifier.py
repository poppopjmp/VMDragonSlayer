...
from .recognizer import PatternRecognizer, PatternMatch, FeatureExtractor
...
class SimilarityClassifier:
    ...
    def classify(self, bytecode_sequence: List[int], 
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(self.pattern_database.match_patterns):
                ...
                matches = self.pattern_database.match_patterns(bytecode_bytes, threshold=0.5)
...
class MLClassifier:
    ...
    def add_training_example(self, bytecode_sequence: List[int], 
                           label: str, context: Optional[Dict] = None):
    ...
    def train(self) -> bool:
        ...
        if len(self.training_data) < 10:
            logger.warning("Insufficient training data (%d examples)", len(self.training_data))
            return False
        ...
        try:
            ...
            logger.debug("Training data: %d examples, %d classes", 
                        len(self.training_data), len(self.class_names))
...
class RuleBasedClassifier:
    ...
    async def classify(self, bytecode_sequence: List[int], 
                      context: Optional[Dict] = None) -> ClassificationResult:
...
class PatternClassifier:
    ...
    async def classify(self, bytecode_sequence: List[int],
                      ...
                      method: Optional[ClassificationMethod] = None) -> ClassificationResult:
    ...
    async def _auto_classify(self, bytecode_sequence: List[int], 
        ...
        similarity_result = self.similarity_classifier.classify(bytecode_sequence, context)
    ...
    async def _hybrid_classify(self, bytecode_sequence: List[int], 
                             context: Dict) -> ClassificationResult:
        ...
        confidence = class_votes[winning_class] / total_weight if total_weight > 0 else 0.0
    ...
    def add_training_data(self, bytecode_sequence: List[int], 
                         label: str, context: Optional[Dict] = None):
