...
from .executor import ExecutionContext, SymbolicValue, SymbolicConstraint, ConstraintType
...
class InstructionLifter:
    ...
    def lift_instruction(self, instruction: Instruction, 
                        context: ExecutionContext) -> LiftingResult:
        ...
        try:
            ...
            logger.debug("Lifted instruction %s at 0x%x in %.3fs", 
                        instruction.mnemonic, instruction.address, result.lifting_time)
    ...
    def _perform_lift(self, instruction: Instruction, 
                     context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_arithmetic(self, instruction: Instruction, 
                        context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_logical(self, instruction: Instruction, 
                     context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_memory(self, instruction: Instruction, 
                    context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_control_flow(self, instruction: Instruction, 
                          context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_stack(self, instruction: Instruction, 
                   context: ExecutionContext) -> LiftingResult:
        ...
        if instruction.mnemonic.upper() == "PUSH" and instruction.operands:
            ...
            sp_value = SymbolicValue(
                name=f"sp_after_{instruction.address}",
                size=32
            )
    ...
    def _lift_comparison(self, instruction: Instruction, 
                        context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_bitwise(self, instruction: Instruction, 
                     context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_vm_specific(self, instruction: Instruction, 
                         context: ExecutionContext) -> LiftingResult:
    ...
    def _lift_generic(self, instruction: Instruction, 
                     context: ExecutionContext) -> LiftingResult:
...
class HandlerLifter:
    ...
    def register_handler(self, address: int, handler_type: str, 
                        complexity: int = 1) -> VMHandlerInfo:
    ...
    def lift_handler(self, instructions: List[Instruction], 
                    initial_context: ExecutionContext) -> LiftingResult:
        ...
        try:
            for instruction in instructions:
                ...
                for context in current_contexts:
                    result = self.instruction_lifter.lift_instruction(instruction, context)
            ...
            logger.debug("Lifted handler with %d instructions -> %d contexts", 
                        len(instructions), len(current_contexts))
    ...
    def get_handler_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "handler_types": list(set(h.handler_type for h in self.handler_info.values())),
