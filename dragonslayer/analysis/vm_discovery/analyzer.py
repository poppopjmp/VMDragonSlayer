...
from ...core.exceptions import VMDetectionError, InvalidDataError
...
class StructureAnalyzer:
    ...
    def analyze_vm_structure(self, binary_data: bytes, vm_structure: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            analysis_results['handler_relationships'] = self._analyze_handler_relationships(
                vm_structure
            )
    ...
    def _analyze_control_flow(self, binary_data: bytes, vm_structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'cfg_nodes': [{'address': node, 'type': cfg.nodes[node].get('type', 'unknown')} 
    ...
    def _analyze_data_flow(self, binary_data: bytes, vm_structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for handler in handlers:
            ...
            for i, instruction in enumerate(instructions):
                ...
                if any(reg in instruction.lower() for reg in ['eax', 'ebx', 'ecx', 'edx']):
    ...
    def _analyze_handler_relationships(self, vm_structure: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_complexity(self, vm_structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        avg_handler_size = total_instructions / handler_count if handler_count > 0 else 0
        
        # Type diversity
        handler_types = set(h.get('type', 'unknown') for h in handlers)
        type_diversity = len(handler_types)
        
        # Control flow complexity
        total_targets = sum(len(h.get('control_flow_targets', [])) for h in handlers)
        avg_targets_per_handler = total_targets / handler_count if handler_count > 0 else 0
    ...
    def _analyze_instruction_patterns(self, binary_data: bytes, vm_structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        most_common_instructions = sorted(instruction_freq.items(), key=lambda x: x[1], reverse=True)[:10]
        most_common_pairs = sorted(instruction_pairs.items(), key=lambda x: x[1], reverse=True)[:10]
        
        return {
            'patterns': {
                'most_common_instructions': [{'instruction': inst, 'count': count} 
                                           for inst, count in most_common_instructions],
                'most_common_pairs': [{'pair': list(pair), 'count': count} 
            ...
            'statistics': {
                ...
                'avg_instruction_frequency': sum(instruction_freq.values()) / len(instruction_freq) if instruction_freq else 0
            }
        }
    
    def get_analysis_capabilities(self) -> List[str]:
        """Get list of analysis capabilities"""
        capabilities = ['complexity_analysis', 'handler_relationships', 'instruction_patterns']
