...
from ...core.exceptions import VMDetectionError, InvalidDataError, handle_exception
...
class VMStructure:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'instruction_patterns': [pattern.hex() for pattern in self.instruction_patterns],
            'confidence': self.confidence
        }
...
class VMDetector:
    ...
    def detect_vm_structures(self, binary_data: bytes) -> Dict[str, Any]:
        ...
        if self.cache_enabled:
            cache_key = hashlib.md5(binary_data).hexdigest()
        ...
        try:
            ...
            self.logger.info(f"VM detection completed: VM detected = {detection_result['vm_detected']}")
    ...
    def _detect_vm_patterns(self, binary_data: bytes) -> Dict[str, Any]:
        ...
        pattern_density = total_matches / len(binary_data) if binary_data else 0
        pattern_confidence = min(pattern_density * 1000, 1.0)  # Scale factor
    ...
    def _analyze_vm_structure(self, binary_data: bytes) -> Dict[str, Any]:
        ...
        structure_confidence = min(structure_count * 0.2, 1.0)
        ...
        for i in range(0, len(binary_data) - 16, 4):
            ...
            for j in range(4):
                if i + j * 4 + 3 < len(binary_data):
                    value = int.from_bytes(binary_data[i + j * 4:i + j * 4 + 4], 'little')
                    values.append(value)
    ...
    def _detect_handlers(self, binary_data: bytes) -> Dict[str, Any]:
        ...
        for i, (pattern, handler_type) in enumerate(handler_patterns):
            ...
            pattern_bytes = pattern.replace(b'\\x', b'').decode('unicode_escape').encode('latin1')
        ...
        # Calculate handler confidence
        handler_confidence = min(len(handlers) * 0.1, 1.0)
    ...
    def _identify_dispatcher(self, binary_data: bytes) -> Dict[str, Any]:
        ...
        dispatcher_confidence = 0.8 if best_dispatcher else 0.0
    ...
    def _combine_detection_results(self, pattern_results: Dict, structure_results: Dict,
                                 handler_results: Dict, dispatcher_results: Dict) -> Dict[str, Any]:
        ...
        vm_detected = overall_confidence >= self.confidence_threshold
        ...
        if vm_detected and handler_results['handlers']:
            vm_structure = VMStructure(
                vm_type=vm_type,
                dispatcher_address=dispatcher_results.get('dispatcher', {}).get('address', 0),
    ...
    def _determine_vm_type(self, pattern_results: Dict, handler_results: Dict) -> VMType:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'pattern_count': sum(len(patterns) for patterns in self.vm_patterns.values())
        }
