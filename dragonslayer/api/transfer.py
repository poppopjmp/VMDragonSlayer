...
class BinaryTransfer:
    ...
    def encode_binary(self, binary_data: bytes, compress: bool = None) -> str:
        ...
        try:
            ...
            if compress and len(binary_data) > 1024:  # Only compress if size > 1KB
                ...
                self.logger.debug(f"Compressed {len(binary_data)} bytes to {len(data_to_encode)} bytes")
    ...
    def stream_encode(self, binary_stream: BinaryIO) -> Iterator[str]:
        ...
        try:
            while True:
                ...
                encoded_chunk = self.encode_binary(chunk, compress=False)  # Don't compress chunks
                yield encoded_chunk
    ...
    def verify_checksum(self, binary_data: bytes, expected_checksum: str, 
    ...
    def estimate_transfer_time(self, data_size: int, bandwidth_mbps: float = 100.0) -> float:
        """
...
def get_transfer_util(chunk_size: int = 1024 * 1024, 
                     enable_compression: bool = True) -> BinaryTransfer:
