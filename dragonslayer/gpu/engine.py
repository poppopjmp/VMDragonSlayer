...
class KernelCache:
    ...
    def get_kernel(self, kernel_source: str, optimization_level: str = "O2") -> Optional[Any]:
    ...
    def cache_kernel(self, kernel_source: str, compiled_kernel: Any, 
                    compilation_time_ms: float, optimization_level: str = "O2") -> str:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_compilation_time = np.mean([entry['compilation_time_ms'] 
                                          for entry in self.cache.values()]) if self.cache else 0
            
            return {
                'cached_kernels': len(self.cache),
                'max_cache_size': self.max_cache_size,
                'cache_utilization_percent': (len(self.cache) / self.max_cache_size) * 100,
...
class GPUEngine:
    ...
    def __init__(self, auto_select_device: bool = True, memory_pool_size_mb: int = 1024):
                ...
                for platform in platforms:
                    ...
                    for device in devices:
                        device_info = GPUInfo(
                            ...
                            compute_capability=(0, 0),  # OpenCL doesn't have this concept
                        ...
                        self.devices.append(device_info)
                        logger.info(f"Detected OpenCL device {device_id}: {device_info.name}")
                        device_id += 1
            except Exception as e:
    ...
    def _select_best_device(self):
        ...
        best_device = max(self.devices, key=lambda d: (
            d.backend == "cuda",
            d.backend == "opencl", 
            d.memory_total_mb,
    ...
    def execute_kernel(self, kernel_source: str, input_data: np.ndarray,
                      grid_size: Tuple[int, ...], block_size: Tuple[int, ...],
                      kernel_name: str = "custom_kernel") -> Tuple[np.ndarray, PerformanceMetrics]:
        ...
        try:
            ...
            if cached_kernel is None:
                ...
                if CUDA_AVAILABLE:
                    ...
                    compiled_kernel = lambda *args: self._cpu_fallback_kernel(input_data)
                
                compile_time = (time.time() - compile_start) * 1000
                self.kernel_cache.cache_kernel(kernel_source, compiled_kernel, compile_time)
            ...
            throughput = data_size_gb / (execution_time / 1000) if execution_time > 0 else 0
